...
// TODO: integrate zalopay and refund method
...
const deliverOrder = async (req: Request, res: Response): Promise<void> => {
  ...
  if (file) {
    result = (
      req.body.fileType === 'zip' ? await uploads(file, `${randomCharacters}.zip`) : await uploads(file, `${config.CLOUD_FOLDER}`)
...
export { requestExtension, deliveryDate, buyerApproveOrder, deliverOrder };
